...
class RoIHead(nn.Module):
    ...
    def roi_grid_pool(self, batch_dict):
        ...
        point_coords = torch.cat(batch_dict['point_coords'], dim=0)
        point_features = torch.cat(batch_dict['point_features'], dim=0)

        global_roi_grid_points, local_roi_grid_points = self.get_global_grid_points_of_roi(rois)  # (BxN, 6x6x6, 3)
        global_roi_grid_points = global_roi_grid_points.view(batch_size, -1, 3)  # (B, Nx6x6x6, 3)

        # mask in range keypoints
        in_range_mask = torch.norm(point_coords[:, :2], dim=-1) < self.model_cfg['det_range']

        xyz = point_coords[in_range_mask]
        ...
        pooled_points, pooled_features = self.roi_grid_pool_layer(
            ...
            features=point_features[in_range_mask].contiguous(),   # weighted point features
        )  # (M1 + M2 ..., C)
    ...
    def get_loss(self, batch_dict):
        ...
        # reg loss
        loss_reg = self.loss_reg(rcnn_reg, tgt_reg, weights=tgt_cls.view(1, -1)).mean()

        loss = loss_cls * self.loss_cls._loss_weight + loss_reg * self.loss_reg._loss_weight
    ...
    def get_detections(self, batch_dict):
        ...
        scores = rcnn_cls[mask]
        coop_vehicles = batch_dict['coop_boxes_in_egoCS']
