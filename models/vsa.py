...
class VoxelSetAbstraction(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        for i, dets in enumerate(dets_list):
            boxes[i, :len(dets)] = dets
        kpt_mask = points_in_boxes_gpu(keypoints, boxes)
        ...
        if (kpt_mask>=0).sum()<2:
            kpt_mask[0, random.randint(0, 1024)] = 0
            kpt_mask[1, random.randint(0, 1024)] = 0
        ...
        if 'bev' in self.model_cfg['features_source']:
            ...
            point_features_list.append(point_bev_features[kpt_mask>=0])

        batch_size, num_keypoints, _ = keypoints.shape
        # new_xyz = keypoints.view(-1, 3)
        # new_xyz_batch_cnt = new_xyz.new_zeros(batch_size).int().fill_(num_keypoints)
        new_xyz = keypoints[kpt_mask >= 0]
        new_xyz_batch_cnt = torch.tensor([(mask >= 0).sum() for mask in kpt_mask], device=new_xyz.device).int()
